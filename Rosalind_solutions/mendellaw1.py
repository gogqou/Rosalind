...
def calc_prob(dom, het, neg):
    prob = 0
    total =float(dom+het+neg)
    prob_picking_dom = dom/total*((dom-1+het+neg)/(total - 1)) + het/total * dom/(total -1) + neg/total * dom/(total-1)
    prob_picking_het = het/total * (het-1)/(total-1)
    prob_picking_het_neg = neg/total * het/(total-1) + het/total * neg/(total-1)
    prob = prob_picking_dom + prob_picking_het*3/4 + prob_picking_het_neg * 1/2
